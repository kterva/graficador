...
export const UNIT_CATEGORIES = {
    ...
    electromagnetism: {
        ...
        baseUnit: 'C', // Base referencial para la categoría
        units: {
            'C': { factor: 1, label: 'coulomb (C)' },
            'A': { factor: 1, label: 'amperio (A)' },
            'V': { factor: 1, label: 'voltio (V)' },
            'Ω': { factor: 1, label: 'ohmio (Ω)' },
            'F': { factor: 1, label: 'faradio (F)' },
            'H': { factor: 1, label: 'henrio (H)' },
            'T': { factor: 1, label: 'tesla (T)' },
            'Wb': { factor: 1, label: 'weber (Wb)' }
        }
    },
    angles: {
        name: 'Ángulos',
        baseUnit: 'rad',
        units: {
            'rad': { factor: 1, label: 'radián (rad)' },
            'deg': { factor: Math.PI / 180, label: 'grado (deg)' }
        }
    }
};
...
export function resolveUnit(unit, category) {
    if (category === 'custom') return { factor: 1, label: unit };
    ...
    if (prefix && base) {
        ...
        // Manejo especial de bases implícitas y explícitas
        ...
        } else if (category === 'electromagnetism') {
            // Manejo de todas las bases de electromagnetismo
            const validElectroBases = ['C', 'A', 'V', 'Ω', 'F', 'H', 'T', 'Wb'];
            if (validElectroBases.includes(base)) {
                validBase = true;
                baseFactor = categoryData.units[base].factor;
                baseName = categoryData.units[base].label.split(' ')[0];
            }
        } else if (category === 'angles' && base === 'rad') {
            validBase = true;
            baseFactor = categoryData.units['rad'].factor;
            baseName = 'radianes';
        }
...
export function convert(value, fromUnit, toUnit, category) {
    ...
    if (category === 'custom') return value;
